...
### November 21, 2025 - Capacitor Native Mobile Integration
...
- **Native Plugins Integrated**: 
  ...
  - @capacitor/filesystem (v7.1.4) - File storage and sharing on native devices
...
- **Hybrid PDF/CSV Export**: Completely refactored exportToPDF() and exportToCSV() functions to work seamlessly in both web (PWA) and native (Android/iOS) environments:
  - **Web**: Maintains original behavior (window.open/download links)
  - **Native**: Saves files via Filesystem API to Directory.Cache, obtains shareable URIs via Filesystem.getUri(), and shares via native Share dialog with files array
  - **PDF**: Base64 encoding with automatic Capacitor decoding (no encoding parameter)
  - **CSV**: UTF-8 text encoding (Encoding.UTF8 parameter)
- **WhatsApp Buttons Removed**: Removed all 7 "Compartilhar no WhatsApp" buttons from simulators (Investimento, Empréstimo, Poupança Programada, Taxa Concorrente, Crédito Rural, Desconto Recebíveis, Conversão de Taxas). Kept only "Copiar Link" button for URL sharing
- **Tutorial Auto-Start Disabled**: Commented out automatic tutorial activation on first app load to reduce user friction. Tutorial remains accessible via "?" button
...
- **Android Platform Ready**: Generated native Android project in `/android/` directory, synced with 6 plugins, ready for .aab generation
...
- **CDI Toggle Correction (Nov 17, 2025)**: Fixed investment simulators to use CDI (not SELIC) in both toggle options. Now correctly offers "CDI (Projeções Multi-Ano)" vs "CDI Fixo" with `settings.cdi` value. Labels updated throughout UI. Poupança continues using SELIC as required by Brazilian regulation. Added localStorage migration to auto-convert old settings format. "Taxas CDI Aplicadas" section now hidden when using CDI Fixo (shows only for multi-year projections).
- **TR (Taxa Referencial) Integration (Nov 17, 2025)**: Added complete TR support to Poupança calculations per Brazilian law. TR field added to Settings menu (default: 1.88% a.a., últimos 12 meses). Poupança formula now correctly calculates: (70% SELIC + TR) when SELIC ≤ 8.5%, or (0.5% a.m. + TR) when SELIC > 8.5%. TR is input as annual rate and automatically converted to monthly for calculations. UI labels updated to show formula dynamically. Automatic localStorage migration ensures backward compatibility.
- **Menu Reorganization & Investment Defaults Updated (Nov 17, 2025)**: Moved "Indicadores Econômicos" card from first position to second-to-last position (before "Configurações") for better workflow organization. Updated default investment rates: LCA/LCI changed from 95% to 82% of CDI, CDB/RDC changed from 110% to 98% of CDI, reflecting more realistic market rates.